  ...
  const startQuiz = () => {
    ...
    const questions = questionWords.map((word) => {
      const distractors = shuffle(
        [...words, ...starterWords].filter(
          (candidate) => candidate.japanese !== word.japanese,
        ),
      )
      ...
      const uniqueDistractors = [
        ...distractors,
        ...fallbackMeanings.filter((meaning) => meaning !== word.meaning),
      ]
        .filter((meaning, index, all) => all.indexOf(meaning) === index)
        .slice(0, 3);
  ...
  const answerQuiz = (answer: string) => {
    setQuizQuestions((current) =>
      current.map((question, index) =>
        index === quizIndex && !question.selectedAnswer
          ? { ...question, selectedAnswer: answer }
          : question,
      ),
    );
    if (
      quizQuestions[quizIndex]?.selectedAnswer ||
      answer !== quizQuestions[quizIndex]?.correctAnswer
    )
      return;
    ...
    <div className="app-shell">
      ...
      {isQuizOpen && (
        <div
          ...
          onMouseDown={(event) =>
            event.target === event.currentTarget && setIsQuizOpen(false)
          }
        >
          <section
          ...
          >
            ...
            {quizFinished ? (
              <div className="quiz-result">
                ...
                <p>測驗完成！</p>
                <button className="quiz-next-button" onClick={startQuiz}>
                  再測一次
                </button>
              </div>
            ) : currentQuiz ? (
              <div className="quiz-body">
                <div className="quiz-progress">
                  第 {quizIndex + 1} / {quizQuestions.length} 題{" "}
                  <span>答對 {quizScore} 題</span>
                </div>
                <div className="quiz-word">
                  <RubyText
                    text={currentQuiz.word.japanese}
                    reading={currentQuiz.word.reading}
                  />
                </div>
                <p className="quiz-prompt">請選擇正確的中文意思</p>
                <div className="quiz-options">
                  {currentQuiz.options.map((option) => {
                    const isSelected = currentQuiz.selectedAnswer === option;
                    ...
                    return (
                      <button
                        ...
                        disabled={Boolean(currentQuiz.selectedAnswer)}
                      >
                        {option}
                      </button>
                    );
                ...
                <div className="quiz-navigation">
                  ...
                  <button
                    ...
                    disabled={!currentQuiz.selectedAnswer}
                  >
