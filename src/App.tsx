...
const starterWords: Word[] = [
  {
    ...
    related: [{ word: '食事', reading: 'しょくじ', note: '用餐、飲食（名詞）' }, { word: '飲む', reading: 'のむ', note: '喝；服用（動詞）' }, { word: '料理', reading: 'りょうり', note: '料理；菜餚（名詞）' }], generatedAt: '2026/09/08',
  },
  {
    ...
    related: [{ word: '予定', reading: 'よてい', note: '預定；計畫（名詞）' }, { word: '約束', reading: 'やくそく', note: '約定；承諾（名詞）' }, { word: '申込み', reading: 'もうしこみ', note: '申請；報名（名詞）' }], generatedAt: '2026/09/08',
  },
  {
    ...
    related: [{ word: '美しい', reading: 'うつくしい', note: '美麗；優美（い形容詞）' }, { word: '清潔', reading: 'せいけつ', note: '清潔；衛生（な形容詞）' }, { word: '汚い', reading: 'きたない', note: '骯髒；不乾淨（い形容詞）' }], generatedAt: '2026/09/08',
  },
  {
    ...
    related: [{ word: '学ぶ', reading: 'まなぶ', note: '學習；習得（動詞）' }, { word: '練習', reading: 'れんしゅう', note: '練習；訓練（名詞）' }, { word: '研究', reading: 'けんきゅう', note: '研究；鑽研（名詞）' }], generatedAt: '2026/09/08',
  },
]
...
function App() {
  ...
  useEffect(() => {
    document.querySelector('.detail-section')?.setAttribute('data-generated-time', `生成時間：${selected?.generatedAt || '尚未記錄'}`)
  }, [selected])
  useEffect(() => {
    ...
    const syncHeight = () => contentGrid.style.setProperty('--card-height', `${detailCard.offsetHeight}px`)
    syncHeight()
  ...
  const translate = async () => {
    ...
    try {
      ...
      const resultWithId = { ...result, id: `${result.japanese}-${Date.now()}`, generatedAt: new Date().toLocaleString('zh-TW', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }) }
  ...
  return (
    <div className="app-shell">
      <main className="workspace">
        <div className="page-tools"><div className="brand"><span className="brand-mark"><BookOpen size={18} /></span><span>日文單字庫</span><span className="version-badge">v0.9</span></div><button className="icon-button" aria-label="開啟設定" onClick={() => setIsSettingsOpen(true)}><Settings size={19} /><span>設定</span></button></div>
        ...
        <div className="content-grid">
          <section className="library-section"><div className="section-heading"><div><h2>我的單字庫</h2></div><span className="result-count">{filteredWords.length} 個單字</span></div><div className="word-list">{filteredWords.map((word) => <button key={word.id} className={`word-row ${selected?.id === word.id ? 'active' : ''}`} onClick={() => setSelected(word)}><span className="word-japanese"><RubyText text={word.japanese} reading={word.reading} /></span><span className="word-meaning">{word.meaning}</span><span className="word-part-of-speech">{word.partOfSpeech}</span><span className="word-level">{word.level}</span></button>)}{filteredWords.length === 0 && <div className="empty-state"><Search size={24} /><p>單字庫裡還沒有這個詞</p><span>按下翻譯，讓 Gemini 幫你查詢</span></div>}</div></section>

          {selected && <section className="detail-section"><div className="detail-topline"><span>單字卡</span><button className={`save-button ${words.some((word) => word.id === selected.id) ? 'saved' : ''}`} onClick={saveWord}><Bookmark size={17} fill={words.some((word) => word.id === selected.id) ? 'currentColor' : 'none'} />{words.some((word) => word.id === selected.id) ? '已儲存單字' : '儲存單字'}</button></div><div className="detail-title"><div className="detail-japanese-group"><div className="word-title-line"><h2><RubyText text={selected.japanese} reading={selected.reading} /></h2><button className="audio-button" aria-label="播放日文單字" title="播放日文單字" onClick={() => speakJapanese(selected.japanese)}><Volume2 size={19} /></button></div><span className="large-level">{selected.level}</span></div><div className="detail-meaning-group"><p>{selected.meaning}</p><span className="part-of-speech">{selected.partOfSpeech}</span></div></div><div className="detail-block example-block"><div className="block-label"><span>例句</span></div>{selected.examples.map((example, index) => <div className="example-item" key={`${example.japanese}-${index}`}><p className="example-japanese"><RubyText text={example.japanese} reading={example.reading} /></p><p className="example-translation">{example.translation}</p></div>)}</div><div className="detail-block"><div className="block-label"><span>搭配詞</span></div><div className="related-list">{selected.collocations.map((item) => <div className="related-item" key={item.word}><strong><RubyText text={item.word} reading={item.reading} /></strong><span>{item.meaning}</span></div>)}</div></div><div className="detail-block"><div className="block-label"><span>相關詞</span></div><div className="related-list">{selected.related.map((item) => <div className="related-item" key={item.word}><strong><RubyText text={item.word} reading={item.reading} /></strong><span>{item.note}</span></div>)}</div></div></section>}
        </div>
      </main>

      <footer><span>2026/09/09&nbsp; | &nbsp;Copyright © 2026 Andy Chiang</span><a href="https://github.com/AndyChiangSH/Kotoba-Base" target="_blank" rel="noreferrer">GitHub <ExternalLink size={13} /></a></footer>
